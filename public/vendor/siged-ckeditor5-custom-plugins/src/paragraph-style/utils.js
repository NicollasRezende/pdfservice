...
function optionDefinition(option, unit, prop) {
	...
	if (option === 'Default') {
		return { model: undefined, title: prop === 'line-height' ? 'Espaçamento de linha padrão' : 'Espaçamento padrão' };
	}
	...
	const named = { 1: 'Simples', 2: 'Duplo' }[sizeName];
	const result = {
		title: named ? named + 'vezes' : unit ? sizeName + unit : sizeName + 'vezes',
		model: size,
